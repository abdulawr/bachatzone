<div class="p-3">
<form class="bg-white p-3 rounded shadow" style="border-left: 4px solid #008693;" method="post" enctype="multipart/form-data" action="action/category">
  ...
  <?php 
  $supp_list = DBHelper::get("SELECT * FROM `product_category`");
  if($supp_list->num_rows > 0){
      while($row = $supp_list->fetch_assoc()){
        ...
        <td>
        <a href="?p=category&ID=<?php echo $row["id"];?>" title="Edit category"><i class="fas fa-pencil-alt text-warning"></i></a>
        </td>
        </tr>
      <?php
      }
...
<script>
    document.getElementById("p_title").innerText = "<?php echo $title;?>category";
</script>
