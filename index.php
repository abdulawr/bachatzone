...
<?php 
$pg = 'index';
include("client/include/nav.php"); 
$fav_array = [];
...
?>
	...
	<?php include("client/include/cart.php");?>
				...
				<div class="tab-content p-t-10">		
					...
					<div class="tab-pane fade show active" id="best-seller" role="tabpanel">
						...
						<div class="wrap-slick2">							
							...
							<?php
							   $prd_IDS = [];
							   $product = DBHelper::get("SELECT * FROM product WHERE isdelete = 0 and status = 1 and views < 1000 and order_no < 50 ORDER by id DESC LIMIT 20");
							   if($product->num_rows > 0){
								   while($row = $product->fetch_assoc()){
									$pt = "client/product-detail?ID=".$row["id"];
									...
									<div class="block2">
										...
										<img height="320" class="rounded-top" src="images/product/<?php echo $row["main_img"];?>" alt="IMG-PRODUCT">
								...
								<button class="btn-addwish-b2 dis-block pos-relative ">
                                    ...
                                    if (isset($_COOKIE["cusID"]) && verifyCustomer()) {
                                        if (in_array($row["id"], $fav_array)) {
                                            ?>
									   <img onclick="addFav('1','<?php echo $row['id']; ?>')" style="opacity:1;" class="icon-heart1 dis-block trans-04" src="client/images/icons/icon-heart-02.png" alt="ICON">
									   <?php
                                        } else {
                                            ?>
										<img onclick="addFav('0','<?php echo $row['id']; ?>')" style="opacity:1;" class="icon-heart1 dis-block trans-04" src="client/images/icons/icon-heart-01.png" alt="ICON">
										<?php
                                        }
                                    }
									?>	
						...
						<div class="wrap-slick2">							
							...
							<?php
							 $idss = rtrim(implode(",",$prd_IDS),",");
							   $product = DBHelper::get("SELECT * FROM product WHERE isdelete = 0 and status = 1 and order_no != 0 and views != 0 and id not in($idss) ORDER by order_no desc,views DESC LIMIT 20");
							   if($product->num_rows > 0){
								   while($row = $product->fetch_assoc()){
									$pt = "client/product-detail?ID=".$row["id"];
									array_push($prd_IDS,$row["id"]);
						...
						<div class="wrap-slick2">							
							...
							<?php
							  $idss11 = rtrim(implode(",",$prd_IDS),",");
							   ...
							   $product = DBHelper::get("SELECT * FROM product WHERE isdelete = 0 and  status = 1 and rating != 0 ORDER by rating DESC LIMIT 20");
							   if($product->num_rows > 0){
								   while($row = $product->fetch_assoc()){
									$pt = "client/product-detail?ID=".$row["id"];
									   ?>
...
<?php include("client/include/footer.php");?>
  ...
  <?php
   include("client/include/links.php");
  ?>
...
<script>
	...
	function addFav(status,prdID){
	  ...
	  var div_bo = $("#spot_"+prdID);
	   $.post("client/action/fav_product.php",
		{
			status: status,
			ID: prdID
		},
		function(data, status11){
			if(data == '1'){
              if(status == '1'){
				div_bo.replaceWith('<div id="spot_'+prdID+'"><img onclick="addFav(0,'+prdID+')" style="opacity:1;" class="icon-heart1 dis-block trans-04" src="client/images/icons/icon-heart-01.png" alt="ICON"></div>')
			  }
			  else{
				div_bo.replaceWith('<div id="spot_'+prdID+'"><img onclick="addFav(1,'+prdID+')" style="opacity:1;" class="icon-heart1 dis-block trans-04" src="client/images/icons/icon-heart-02.png" alt="ICON"></div>')
			  }
			}
		});
	}
