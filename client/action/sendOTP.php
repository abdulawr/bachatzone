...
if(isset($_POST["type"]) && $_POST["type"] == "otp_ver"){
    ...
    $check = DBHelper::get("SELECT * FROM `account_email_otp` WHERE code = '{$opt}' and email = '{$email}' and type = 1 limit 1");
    if($check->num_rows > 0){
    ...
    DBHelper::set("DELETE FROM `account_email_otp` WHERE email = '{$email}'");
    DBHelper::set("INSERT INTO `account_email_otp`(`code`, `type`, `email`) VALUES ('{$otp}',1,'{$email}')");
